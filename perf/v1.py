...
def get_per_token_logps(logits, input_ids):
    ...
    per_token_logps = []
    for logits_row, input_ids_row in zip(logits, input_ids):  # along 0th dim zipping : iterate over B batch
        log_probs = logits_row.to(torch.float32).log_softmax(dim=-1)  # logits_row[L, V] -> log_probs[L, V] log_smax for all V words
        token_logpbs = torch.gather(log_probs, dim=1, index=input_ids_row.unsqueeze(1)).squeeze(1) # logpbs for mentioned token_id
        per_token_logps.append(token_logpbs)    # token_logpbs[L] 
...
def get_ref_per_token_logps(input_ids, attention_mask, prompt_len):
    ...
    # .cpu() offload to cpu to free up vram and no other mega ops needed on this  
    return logpbs[:, prompt_len - 1: ].detach().cpu()
...
generation_config = GenerationConfig(
    ...
    min_new_tokens=max_new_token_len,
    temperature=0.8,
    do_sample=True,   
    top_p=0.95,
    num_return_sequences=num_gen_Q,
    pad_token_id=tokenizer.pad_token_id,
    eos_token_id=None, # disable EOS so it doesn't stop early
)
...
def GRPO_step(batch):
    ...
    advantages = batch["rewards"].to(model.device).unsqueeze(1)   # (B, 1)
...
def profile_grpo_step(batch):
    ...
    # actual profile
    # emit_nvtx() connects PyTorch's record_function to Nsight Systems
    with torch.autograd.profiler.emit_nvtx():
        # Run 4 iterations as you did before
        for i in range(4):
            # nvtx.range_push/pop creates a named range visible in Nsight
            ...
            nvtx.range_pop() # End Step_i

    
    print("Profiling complete. Process exiting.")
    sys.exit(0)
...
while(True):
    ...
    if compute_gen_logps:
        with torch.inference_mode():
            ...
            gen_logps = get_per_token_logps(logits, shifted_ids)[:, prompt_len - 1: ].detach().cpu()
